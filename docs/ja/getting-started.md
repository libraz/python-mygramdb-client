...
## MygramDB v1.10 に関する注記

サーバーを更新する際に注意が必要な変更が 2 つあります。

- **管理コマンドにトークンが必要**。サーバーの TCP リスナーがループバック限定
  でない場合は `ClientConfig.admin_token` を設定してください。設定しておけば
  クライアントが接続時と透過的な再接続時に `AUTH` を送信します。設定がないと
  管理コマンドは `AuthenticationError` で失敗します。TCP 経路はトークンを
  暗号化しないため、そのリスナーは信頼できるネットワーク内か、TLS を終端する
  プロキシの背後に置いてください。
- **エラーが数値コードを持つ**。メッセージの文字列一致ではなく
  `ServerError.error_code`（`ErrorCode` を参照）で分岐してください。
  `ServerNotReadyError` と `ServerBusyError` はリトライで解消しうる状態を表し、
  `RetryPolicy` は既定でこれらをリトライします。

MygramDB v1.9 機能（`FacetOptions.offset` と `FacetResponse.total_count` による
ファセットのページネーション、`filter_conditions` による比較フィルタ）、
v1.7 機能（マルチデータベース、`search_raw`、ランタイム変数、オンデマンド同期）、
v1.6 機能（ファジー検索、ハイライト、ファセット、BM25）も継続してサポートされます。
